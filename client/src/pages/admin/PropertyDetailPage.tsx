...
import { MapPin, Bed, Bath, Calendar, User, Phone, Mail, CheckCircle, XCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function PropertyDetailPage() {
  ...
  const handleAssignEvaluator = () => {
    ...
    const evaluator = mockEvaluators.find(e => e.id === selectedEvaluator);
    toast({
      title: "Evaluator Assigned",
      description: `Property assigned to ${evaluator?.name} for inspection.`,
    });
  ...
  const handleApprove = () => {
    toast({
      ...
      description: "The property has been approved and is now live for tenants.",
    });
    setTimeout(() => {
      if (fromPage === "dashboard") {
        setLocation("/admin/dashboard");
      } else {
        setLocation(`/admin/properties?tab=${fromTab}`);
      }
    }, 1500);
  };
  ...
  const handleReject = () => {
    ...
    toast({
      ...
      description: "The landlord has been notified with the reason for rejection.",
      variant: "destructive",
    });
        ...
        {property.status === "pending" && (
          <div className="space-y-3">
            <Button
              ...
              data-testid="button-assign-evaluator"
            >
              Assign to Evaluator
            </Button>
            <Button
